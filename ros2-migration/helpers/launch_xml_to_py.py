...
def translate(launch_xml_path: str) -> str:
    ...
    for elem in root:
        if elem.tag == "arg":
            n = elem.attrib["name"]
            d = elem.attrib.get("default", elem.attrib.get("value", ""))
            declared.append(
                f"DeclareLaunchArgument({py_str(n)}, default_value={py_str(d)}),"
            )
        elif elem.tag == "node":
            actions.extend(translate_node(elem))
        elif elem.tag == "include":
            f = elem.attrib.get("file", "")
            actions.append(
                f"IncludeLaunchDescription(PythonLaunchDescriptionSource({subst(f)})),"
            )
        elif elem.tag == "rosparam":
            # global rosparam — apply via /** wildcard; left as TODO
            actions.append(f"# TODO global rosparam: {ET.tostring(elem).decode()}")
        elif elem.tag == "param":
            actions.append(f"# TODO global <param>: {ET.tostring(elem).decode()}")
